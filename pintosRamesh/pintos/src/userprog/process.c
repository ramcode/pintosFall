...
static struct semaphore sema_lock;
...
struct child_thread *child_thread = palloc_get_page(0);
...
char *temp;
char *prg_name = strtok_r(fn_copy2, " ", &temp) ;
...
struct file *f  = file_open(prg_name);
if(!f){return -1;}
file_deny_write(f);
child_thread->pid = tid;
child_thread->exited = false;
child_thread->waiting = false;
child_thread->exit_status = -1;
list_push_back(&(current_thread->child_threads), &(child_thread->elem));
child_t->parent_pid = current_thread->tid;
