...
bool alarm_Insert_Less(struct list_elem *left, 
...
timer_sleep (int64_t ticks) 
{
  ...
  current_threads()->count_ticks = start+ticks;
  list_insert_ordered(&sleep_list,current_threads()->elem,
						alarm_Insert_Less,NULL);
	thread_yield();
  )
  ASSERT (intr_get_level () == INTR_ON);
